...
|   | Implementing 20 design principles | 🧡 | From the beginning I wanted to create an application with **One Primary action per screen**. While later on implementing other design principles from [Principles of User Interface Design](http://bokardo.com/principles-of-user-interface-design/), even though not all design principles are applicable in this prototype. I do feel like I could have done a better job in creating a better user experience design with these principles, but in the end there just wasn't enough time. The hardest one I found **Clarity is job #1**, because you don't want a overload of textual content on the homepage, but want to make clear to the user what kind of application they use. I also found it hard to implement certain design decisions into the concept, when working in a group. This was because everybody had an opinion on something, so we had to compromise to make sure someone else also reached their learning goals. |
